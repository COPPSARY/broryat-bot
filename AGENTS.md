...
- `bot/services/ai/` — `base.py` (`AIProvider` ABC + `AIProviderError`), `prompt.py` (shared prompt builder encoding the PRD's scam categories), `gemini_provider.py` (the only concrete provider so far), `factory.py` (`get_ai_provider(settings)` — the sole seam other code depends on; add new providers here, not by branching in callers).
...
**Cambodia AI Scam Shield** is an AI-powered cybersecurity platform, starting as a Telegram bot and expanding into a community threat-intelligence platform for Cambodia. Read `docs/requirements.md` in full before implementing features — it is the source of truth for scope and behavior. Key points:
