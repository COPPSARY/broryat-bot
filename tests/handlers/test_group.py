...
async def test_text_with_link_is_scanned_and_defaults_to_khmer():
    ...
    assert pipeline.run.call_args.kwargs == {"use_ai": False}
...
async def test_vt_malicious_url_replies_with_a_secretary_style_warning():
    update = _update(text="check this out https://example.com")
    pipeline = _pipeline()
    pipeline.run.return_value = _scan_result(
    ...
    await handle_group_message(update, context, pipeline, _group_pref_repo(language="en"), group_scan_enabled=True)

    placeholder = update.message.reply_text.return_value
    placeholder.edit_text.assert_awaited_once()
    assert placeholder.edit_text.await_args.args[0].startswith(
...
async def test_malicious_group_warning_stays_visible(mock_sleep):
    update = _update(text="check this out https://example.com")
    pipeline = _pipeline()
    pipeline.run.return_value = _scan_result(
        vt_url=VTUrlVerdict(url="https://example.com", status="malicious", malicious_count=5, total_engines=10)
...
async def test_vt_malicious_file_replies_without_removing_the_group_message():
    document = MagicMock()
...
async def test_clean_result_stays_silent_after_the_scan(mock_sleep):
    update = _update(text="check this out https://example.com")
    pipeline = _pipeline()
    pipeline.run.return_value = _scan_result(risk_level=RiskLevel.SAFE)
    context = _context()
...
async def test_group_warning_never_includes_the_ai_freeform_message():
    update = _update(text="check this out https://example.com")
    pipeline = _pipeline()
    pipeline.run.return_value = _scan_result(
        risk_level=RiskLevel.MEDIUM,
        message="THIS EXACT AI SENTENCE SHOULD NOT APPEAR",
        vt_url=VTUrlVerdict(url="https://example.com", status="suspicious", malicious_count=3, total_engines=70),
    )
    context = _context()
...
async def test_suspicious_virustotal_result_replies_with_a_secretary_style_warning(mock_sleep):
    update = _update(text="check this out https://example.com")
    pipeline = _pipeline()
    pipeline.run.return_value = _scan_result(
        risk_level=RiskLevel.MEDIUM,
    ...
    placeholder = update.message.reply_text.return_value
    text = placeholder.edit_text.await_args.args[0]
    assert text.startswith("This response is generated by Broryat Bot.")
    assert "3/70" in text
