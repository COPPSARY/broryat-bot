...
_MARKER_PATTERN = re.compile(r"risk\s*:\s*(SAFE|LOW|MEDIUM|HIGH|UNKNOWN)", re.IGNORECASE)
...
def parse_ai_response(text: str) -> tuple[RiskLevel, str]:
    ...
    The marker is always plain English (SAFE/LOW/MEDIUM/HIGH/UNKNOWN) so parsing never
    depends on the language of the user-visible message, which may be fully localized.
    """
