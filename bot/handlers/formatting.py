...
_VT_ALERT = {
    "en": (
        ...
        "Broryat may make mistakes. {advice}"
    ),
    "km": (
        "សារនេះ ឆ្លើយតបដោយ Broryat Bot\n\n"
        "⚠️ ឯកសារ ឬតំណនេះមានមេរោគ ឬខ្លឹមសារគ្រោះថ្នាក់។\n"
        "ការរកឃើញរបស់ VirusTotal៖ {malicious}/{total}{names}\n\n"
        "Broryat អាចធ្វើការវាយតម្លៃខុស។ {advice}"
    ),
}
...
_VT_ALERT_ADVICE = {
    "en": "Please verify before taking action.",
    "km": "សូមផ្ទៀងផ្ទាត់មុននឹងធ្វើសកម្មភាព។",
}


def format_vt_alert(
    result: ScanResult, language: str, *, ask_to_delete: bool = False
) -> str:
    """Format the concise VirusTotal-only warning used for flagged scans."""
    verdict = result.vt_file or result.vt_url
    if verdict is None:
        return _ANALYSIS_FAILED[language]

    names = (
        f"\n{_DETECTED_AS[language]}: {escape(', '.join(verdict.detection_names[:3]))}"
        if verdict.detection_names
        else ""
    )
    alert = _VT_ALERT[language].format(
        malicious=verdict.malicious_count,
        total=verdict.total_engines,
        names=names,
        advice=_VT_ALERT_ACTION[language] if ask_to_delete else _VT_ALERT_ADVICE[language],
    )
    return alert
